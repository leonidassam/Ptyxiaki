...
numberOfNodes <- c( 40, 80, 120)
numberOfCheckInNodesHBF <- list( )
numberOfCheckInNodesHDF <- list( )
numberOfCheckInNodesHCF <- list( )
numberOfCheckInNodesPCI <- list( )

for( n in numberOfNodes) {

  for( z in 1:iter) {
    
    g <- sample_pa( n, m = 3, directed = TRUE)
  ...
  for( i in 1:length( results[[1]])) {
    if( results[[1]][[i]] == 1) {
      numberOfCheckInNodesHBF <- c( numberOfCheckInNodesHBF, i)
      break
    }
  }
  for( i in 1:length( results[[2]])) {
    if( results[[2]][[i]] == 1) {
      numberOfCheckInNodesHDF <- c( numberOfCheckInNodesHDF, i)
      break
    }
  }
  for( i in 1:length( results[[3]])) {
    if( results[[3]][[i]] == 1) {
      numberOfCheckInNodesHCF <- c( numberOfCheckInNodesHCF, i)
      break
    }
  }
  for( i in 1:length( results[[4]])) {
    if( results[[4]][[i]] == 1) {
      numberOfCheckInNodesPCI <- c( numberOfCheckInNodesPCI, i)
      break
    }
  }
  ...
  fname <- c( "Cover Rate Function", "BA network", n, "nodes", ".png")
  plotCoverRateFunction( g, results[[1]], results[[2]], results[[3]], results[[4]], toString( fname))
...
fname <- c( "Number of check in nodes", "BA network", n, "nodes", ".png")
plotNumberOfCheckInNodes( g, numberOfNodes, numberOfCheckInNodesHBF, numberOfCheckInNodesHDF, numberOfCheckInNodesHCF, numberOfCheckInNodesPCI, toString( fname))